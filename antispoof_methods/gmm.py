...
# configs - init
logging.basicConfig(format='%(asctime)s.%(msecs)03d %(levelname)-8s %(message)s', level=logging.WARNING, datefmt='%Y-%m-%d %H:%M:%S')
...
#             xx[i,j] = k  # change to x[k] if indexing xy
...
def train_gmm(data_label, features, train_keys, train_folders, audio_ext, dict_file, ncomp, init_only=False):
    ...
    if exists(partial_gmm_dict_file):
        ...
        for k, train_key in enumerate(train_keys):
            ...
            for nf, file in enumerate(files_subset):
        ...
        print(gmm.persistent_attributes)
        ...
        #     pickle.dump(gmm.get_params(), f)

        gmm.save(partial_gmm_dict_file)
